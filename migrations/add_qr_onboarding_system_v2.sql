...
DROP POLICY IF EXISTS "Admins can manage QR codes" ON onboarding_qr_codes;
DROP POLICY IF EXISTS "Anyone can read active QR codes" ON onboarding_qr_codes;
DROP POLICY IF EXISTS "Admins can manage assessments" ON onboarding_assessments;
DROP POLICY IF EXISTS "Anyone can read active assessments" ON onboarding_assessments;
...
DROP POLICY IF EXISTS "Anon can read active QR codes" ON onboarding_qr_codes;
DROP POLICY IF EXISTS "Anon can read active assessments" ON onboarding_assessments;
...
-- Grant specific permissions to anonymous users (for public QR scanning)
...
COMMENT ON COLUMN user_profiles.birth_date IS 'Employee date of birth for records';

-- Success message
DO $$
BEGIN
  RAISE NOTICE 'QR Onboarding System V2 migration completed successfully!';
  RAISE NOTICE 'Tables created: onboarding_qr_codes, onboarding_assessments, onboarding_responses';
  RAISE NOTICE 'Default assessment added with 5 questions';
  RAISE NOTICE 'RLS policies configured for public and authenticated access';
END $$;
